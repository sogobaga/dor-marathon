...
- **詳細頁**：Detail DTO 新增 `item_mode`＋`variants`。多品項模式：**隱藏底部固定 CTA**（商家層級 cta_url 不用），在 `video_urls` 區塊之後（捲動內容最底部）渲染「品項」區塊：標題「品項」＋每個品項一張卡：上方全寬 2:1 banner 圖（`aspectRatio 2/1`、`objectFit:cover`，與商家 banner 同比例；無圖時不渲染圖片區、不留占位——v842 依使用者要求由 72px 正方縮圖改為 2:1）＋下方名稱（粗體）／描述（`pre-line`、小字、次要色）／「前往」按鈕（primaryFullBtn 風格、**寬版占滿整列**——v843 使用者回饋左下角小鈕右手難按，開新分頁 `noopener,noreferrer`；無 cta_url 且未鎖定時不顯示按鈕）。單一品項模式：**完全維持現狀**（不渲染 variants 區塊，即使資料有）。

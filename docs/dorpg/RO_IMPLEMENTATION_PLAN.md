...
## ⚠️ 使用者拍板（2026-09-17）——以下決定**覆蓋**本計畫的建議，實作以 `P5_CONTRACT.md` 為準

| 項目 | 使用者決定 | 對本計畫的影響 |
|---|---|---|
| 怪物數值 | **絕對值**，正式上線後不隨玩家戰力縮放（未來：地圖移動隨機遇敵、共同挑戰 BOSS、可單人可組隊、怪物特徵要能引發攻略討論） | 推翻決策點 1 的建議 A；排在**第二輪**（需重做平衡） |
| 升級來源 | 主要來自**跑步任務／活動**；打怪與純跑步只給零星 EXP | §6 的 Job Lv 整段作廢；戰鬥 EXP 政策留待第二輪獎勵設計 |
| 等級→防禦 | Lv≤50 每級 +0.5、Lv≥51 每級 +0.35，其餘成長靠裝備與技能 | 取代現行 floor(Lv/2) |
| 職業制度 | 初期**不做轉職、不做職業等級**；六職業改用 DOR 正名（輕騎士／弓箭手／重騎士／聖職者／商人／魔法師，各兩條發展路線） | §4 的 RO 職業表與 Job Lv 加點作廢；不照抄 RO 名稱與數字 |
| 配點 | 依 RO 給點規則（Lv1 48 點、每級 ⌊(L−1)/5⌋+3）、素質上限＝等級、+1／Max／還原預設、測試等級可自設 | 取代 40 點固定配額 |
| 技能 | 六職業各 10 個（兩路線各 5）共 60，職業不共用，先文字呈現，交付 Excel；每級給技能點（1 點／級） | 取代決策點 8 的「六個技能」 |
| ②硬防 ③MATK ⑦HP/SP 係數 | 維持現行／不做 | 同建議 |
| ④暴擊 | 浮動倍率 1.75–2.25，仍扣防 | 取代固定 2.0 |
| ⑤屬性 | 技能帶屬性、怪物有相剋屬性、命中相剋 +25% | 取代 4 級表 |
| ⑥體型 | 只展示，武器系統時再用 | 同建議 |
| ⑨整十階梯 | STR＋INT 都做 | 取代「只做 STR」 |
| ⑩防弊 | 節流＋合理性檢查，未來再考慮伺服器逐幀重算 | 同建議 |

第一輪（P5）範圍與介面：`P5_CONTRACT.md`、`P5_WIRE.md`；技能表：`DORPG_職業技能表_v1.xlsx`／`SKILLS_v1.md`。

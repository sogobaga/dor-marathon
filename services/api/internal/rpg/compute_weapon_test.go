...
// 每個欄位分開驗證（其餘欄位/素質留 0，孤立變數），比照 compute_test.go 既有風格。eq=nil 的既有
// 行為由 compute_test.go 全部既有測試在不改動下繼續覆蓋（沒有任何一個既有測試設定
// ComputeInput.Equip，等同持續驗證「nil＝零改動」）。
//
// DORPG P8：ComputeInput.Weapon 欄位已被 Equip *EquipBonus 取代（見 compute.go 該欄位註解），
// Compute() 內部改吃 AggregateEquipment(weapon, armors) 彙總後的結果——這裡全部測試改用
// equipOf(wp) 這個小 helper 組出「只有武器、沒有任何防具」的 EquipBonus，驗證 CONTRACT
// 「只有武器時結果與 P7 逐位元一致」（armor.go AggregateEquipment 檔頭註解同款斷言）。
package rpg

import "testing"

// equipOf armors 傳 nil（沒有任何防具/飾品）：只有武器貢獻的 EquipBonus，供本檔逐欄位測試沿用
// P7 時代「ComputeInput{Weapon: &wp}」的等效寫法。
func equipOf(wp WeaponProfile) *EquipBonus {
...
func TestCompute_Weapon_IntBonus(t *testing.T) {
	...
	d := Compute(cfg, ComputeInput{Equip: equipOf(wp)})
...
func TestCompute_Weapon_AtkPct(t *testing.T) {
	...
	d := Compute(cfg, ComputeInput{Stats: Stats{Str: 10}, Equip: equipOf(wp)})
...
func TestCompute_Weapon_MatkPct(t *testing.T) {
	...
	d := Compute(cfg, ComputeInput{Stats: Stats{Int: 10}, Equip: equipOf(wp)})
...
func TestCompute_Weapon_DefPct(t *testing.T) {
	...
	d := Compute(cfg, ComputeInput{Stats: Stats{Agi: 10}, Equip: equipOf(wp)})
...
func TestCompute_Weapon_MdefPct(t *testing.T) {
	...
	d := Compute(cfg, ComputeInput{Stats: Stats{Vit: 10}, Equip: equipOf(wp)})
...
// TestCompute_Weapon_FloorsOnlyWhenEquipped 驗證「全部 floor 成整數」只在裝備時發生——不裝備
// 任何東西時既有測試（compute_test.go 全部案例）從未設定 Equip，等同持續驗證 eq==nil 時
...
func TestCompute_Weapon_FloorsOnlyWhenEquipped(t *testing.T) {
	cfg := DefaultConfig()
	withWeapon := Compute(cfg, ComputeInput{Stats: Stats{Str: 10}, Equip: equipOf(WeaponProfile{})})

...
// 不可多——缺 key 由 normalizeCheerLayout 端補預設值，這裡只需擋未知 key。
func TestIsCheerLayoutJSON(t *testing.T) {
	const valid3 = `{"01":{"dx":0,"dy":0,"scale":1},"02":{"dx":12.3,"dy":-45.6,"scale":1.5},"03":{"dx":-300,"dy":300,"scale":4}}`
	const valid8 = `{"01":{"dx":0,"dy":0,"scale":1},"02":{"dx":12.3,"dy":-45.6,"scale":1.5},"03":{"dx":-300,"dy":300,"scale":4},"04":{"dx":0,"dy":0,"scale":1},"05":{"dx":0,"dy":0,"scale":1},"06":{"dx":0,"dy":0,"scale":1},"07":{"dx":0,"dy":0,"scale":1},"08":{"dx":0,"dy":0,"scale":1}}`
	...
	cases := []struct {
		...
		{"空字串合法(清空用預設)", "", true},
		{"只有3key(01/02/03)合法(缺key由normalizeCheerLayout補預設)", valid3, true},
		{"完整8key合法", valid8, true},
		{"壞JSON", `{"01":{"dx":0`, false},
		{"非物件(陣列)", `[1,2,3]`, false},
		{"含未知key(09)不合法", `{"01":{"dx":0,"dy":0,"scale":1},"09":{"dx":0,"dy":0,"scale":1}}`, false},

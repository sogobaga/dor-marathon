...
type Config struct {
	...
	ECPayEnv           string // stage | prod（僅此為 prod 且來源網域列在 ECPayProdHosts 才會用正式特店，見下）
	...
	// 為避免 UAT 測試不慎刷到真錢，正式特店僅在 ECPayEnv=prod 且請求來源網域明確列在 ECPayProdHosts 時才會被選用
	// （見 internal/payment.MultiConfig.ResolveByHost，故障安全：任何不確定情況一律退回測試特店）。
	...
	ECPayProdHosts      []string // 允許使用正式特店的網域（逗號分隔，預設 www.dor.tw,dor.tw）
...
func Load() *Config {
	return &Config{
		...
		ECPayProdHosts:      splitCSV(getEnv("ECPAY_PROD_HOSTS", "www.dor.tw,dor.tw")),

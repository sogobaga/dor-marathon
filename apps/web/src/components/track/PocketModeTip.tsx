...
// 首次開跑提示（CONTRACT.md §2.5）：每裝置只顯示一次（localStorage 記旗標），內容固定：
// 「跑步時請讓螢幕開著；要放口袋請按『口袋模式』。按側邊鍵鎖屏會讓 GPS 暫停，解鎖後會自動接續。」
// active＝true（開跑）時檢查旗標；已看過就永遠不再顯示，不受這次追蹤結束/重開影響。
const SEEN_KEY = 'dor_track_pocket_tip_seen'

export default function PocketModeTip({ active }: { active: boolean }) {

...
export const SETTINGS_SPECS: SettingSpec[] = [
  ...
  },
  {
    ...
    help: '控制「跑後總結／歷史紀錄詳情」頁的政府「揮汗有禮」活動「截圖模式」按鈕對前台玩家的可見性（500.gov.tw）。'
      + '2026-09-06 起改為：達標（單次 5 公里或 30 分鐘）就能開啟一個全螢幕原始紀錄畫面，由使用者自己用手機'
      + '系統截圖鍵擷取整個畫面上傳——純前端功能（無對應後端 API），先只給超管看，備妥後再開放。'

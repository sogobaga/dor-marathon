...
export const SETTINGS_SPECS: SettingSpec[] = [
  ...
  },
  {
    ...
    help: '控制「跑後總結／歷史紀錄詳情」頁的政府「揮汗有禮」活動區塊對前台玩家的可見性（500.gov.tw）。'
      + '2026-09-06 起改為：不再另開全螢幕「截圖模式」畫面——達標（單次 5 公里或 30 分鐘）時直接在'
      + '這兩個原本就有的紀錄畫面上（含地圖每公里號碼標記）顯示提示與「前往運動部活動網頁，上傳截圖」'
      + '按鈕，由使用者自己用手機系統截圖鍵擷取整個畫面上傳——純前端功能（無對應後端 API），先只給'
      + '超管看，備妥後再開放。「off」比其他狀態多一層：連超管旁路都關閉，整批收納此工具時用。',
    options: [
  ...
  // 透過這個數字來調整虛擬選手的能力」。step=0.01 允許輸入小數——這個欄位刻意不是整數，若沿用預設 step=1，
  // 後台儲存時會被 Math.round() 捨成整數，0.75 會存不進去。原本 step=0.05；migration 192（使用者原話「頻率和
  // 距離再降25%」）把正式站值乘 0.75 成 0.56，0.05 的顆粒度存不住，改成 0.01 讓 DB 值與後台儲存顆粒度一致。
  {
    ...
    help: '同時調整虛擬選手的「出勤頻率」與「單次跑步距離」，一個數字掌控整體活躍程度。1＝現行標準能力；'
      ...
      + '0.56（2026-09-21 起正式站值，＝0.75 再降 25%）＝頻率與距離各約 56%、月里程期望值約 31%；'
      + '數字越小虛擬選手越安靜，越大則越活躍（上限 3）。改動後於下一個整點批次生效，不影響已產生的歷史活動。',
    min: 0.1, max: 3, step: 0.01, def: '1',
  },

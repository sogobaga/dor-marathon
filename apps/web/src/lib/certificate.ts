...
// 將完賽證明繪製成 PNG 並觸發下載
export function downloadCertificate(cert: Certificate) {
  const W = 1240
  const H = 877
  const cx = W / 2
  ...
  ctx.fillText('茲證明', cx, 400)
  ctx.fillStyle = GOLD
  ctx.font = `800 64px ${CJK}`
  ctx.fillText(cert.name || '跑者', cx, 472)

  // 賽事名稱
  ctx.fillStyle = 'rgba(255,255,255,.85)'
  ctx.font = `500 30px ${CJK}`
  ctx.fillText(`完成「${cert.race_title}」`, cx, 540)
  if (cert.group_name) {
    ctx.fillStyle = 'rgba(255,255,255,.55)'
    ctx.font = `400 22px ${CJK}`
    ctx.fillText(cert.group_name, cx, 578)
  }
  ...
  stats.forEach(([label, value], i) => {
    ...
    if (i < 2) {
      ...
      ctx.lineWidth = 1
      ctx.beginPath()
      ctx.moveTo(100 + colW * (i + 1), baseY - 36)
      ctx.lineTo(100 + colW * (i + 1), baseY + 24)
      ctx.stroke()
    }
  })
  ...
  canvas.toBlob((blob) => {
    ...
    a.href = url
    a.download = `完賽證明_${cert.race_title}.png`

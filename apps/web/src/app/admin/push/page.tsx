...
export default function AdminPushPage() {
  ...
  async function send() {
    ...
    const channels = [...(doPush ? ['push'] : []), ...(doEmail ? ['email'] : [])] as ('push' | 'email')[]
    if (!channels.length) { setErr('請至少勾選一個發送頻道（推播／Email）'); return }
  ...
  return (
    <div>
      ...
      <p style={{ fontSize: 13, color: 'var(--tx-dim)', lineHeight: 1.7, margin: '0 0 16px', maxWidth: 640 }}>
        發送 Web Push／Email 給指定對象。<b>點擊網址</b>留空預設導向首頁 <code>/</code>。Email 需管理員設好 SMTP 後才會實際寄出（未設定則只發推播）。
      </p>
        ...
        <div style={{ display: 'flex', gap: 12, marginTop: 14, alignItems: 'center', flexWrap: 'wrap' }}>
          ...
          {result && (
            <span style={{ fontSize: 13, color: 'var(--tx-dim)' }}>
              對象 <b style={{ color: 'var(--tx)' }}>{result.recipients}</b> 人　·　推播 <b style={{ color: 'var(--fug)' }}>{result.push_sent}</b>／失敗 <b style={{ color: result.push_failed > 0 ? 'var(--hunt)' : 'var(--tx-dim)' }}>{result.push_failed}</b>　·　Email <b style={{ color: 'var(--fug)' }}>{result.email_sent}</b>／失敗 <b style={{ color: result.email_failed > 0 ? 'var(--hunt)' : 'var(--tx-dim)' }}>{result.email_failed}</b>
            </span>
          )}
        </div>

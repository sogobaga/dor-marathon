...
export default function NewRaceModal({
  ...
  const [mode, setMode] = useState<EventMode>('general')
  ...
  const [entryFeeNtd, setEntryFeeNtd] = useState('0')

  // 子資料
  const [groups, setGroups] = useState<RaceGroup[]>([emptyGroup(0)])
  const [addons, setAddons] = useState<RaceAddon[]>([])
  ...
  function applyPreset(i: number, presetId: string) {
    ...
    updateGroup(i, {
      name: p.name,
      target_distance_km: p.default_distance_km ?? null,
    })
  }
  ...
  async function submit() {
    ...
    setSaving(true)
    try {
      await adminRacesApi.create(token, buildPayload())
      onCreated()
    } catch (e: any) {
  ...
  return (
    <div style={overlay} onClick={onClose}>
      <div style={panel} onClick={(e) => e.stopPropagation()}>
          ...
          {MODES.map((m) => (
            <button
              ...
              style={{
                ...
                background: mode === m.v ? 'rgba(45,212,150,.08)' : 'var(--bg-2)',
                color: 'var(--tx)',
              }}
            >
        ...
        <div style={{ minHeight: 280, maxHeight: '46vh', overflowY: 'auto', paddingRight: 4 }}>
          {tab === 'basic' && (
            <div style={col}>
              <Field label="賽事名稱 *">
          ...
          {tab === 'groups' && (
            <div style={col}>
              {isRandom && (
                <div style={hint}>分組對抗模式：報名時隨機分配、賽前不公開。以下分組即為對抗陣營。</div>
              )}
              {groups.map((g, i) => (
                <div key={i} style={card}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
                    <strong style={{ fontSize: 13 }}>分組 {i + 1}</strong>
                    {groups.length > 1 && (
                      <button onClick={() => removeGroup(i)} style={linkBtn}>移除</button>
                    )}
                  </div>
                  <Row>
                    <Field label="分組名稱">

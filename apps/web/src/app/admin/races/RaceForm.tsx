...
// 面額值解析（migration 149）：優先用序號組結構化 face_value（>0＝已設定，如 1000）；未設定(0)時才回退
// 舊制從名稱/品項解析數字。統一給 computeSerialDenomCalcs／inline 提示／目標反推共用，避免同一序號組
// 在不同面板算出不一樣的面額值（兩處分岔）。fromFaceValue 供顯示端決定標註文字（序號組面額 vs 自名稱解析）。
function resolveFaceValue(g: RewardSerialGroup): { value: number | null; fromFaceValue: boolean } {
  if (g.face_value > 0) return { value: g.face_value, fromFaceValue: true }
  return { value: parsePointValue(g.name || g.item_label || ''), fromFaceValue: false }
}
...
function collectLinePointFaces(items: RewardItem[], groups: RewardSerialGroup[]): ReverseFaceInput[] {
  ...
  items.forEach((item) => {
    ...
    effectiveDenoms(item).forEach((d) => {
      ...
      const { value } = resolveFaceValue(g)
...
function SerialDenomFields({ item, groups, costPerPoint, onChange }: {
  ...
  const merchantGroups = groups
    ...
    .slice()
    .sort((a, b) => {
      const va = resolveFaceValue(a).value
      const vb = resolveFaceValue(b).value
            ...
            return (
              <div key={g.id} style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                ...
                {costPerPoint != null && weight > 0 && (() => {
                  ...
                  const { value: pointValue, fromFaceValue } = resolveFaceValue(g)
                  if (pointValue == null) {
                    return <span style={{ ...calcHint, color: 'var(--hunt)' }}>⚠ 無法解析面額，請在序號組設定面額或於名稱中包含點數數字</span>
                  }
                  const expectedPoints = denomExpectedPoints(pointValue, g.grant_count, actualProb)
                  return (
                    <span style={calcHint}>
                      面額值 {pointValue}（{fromFaceValue ? '序號組面額' : '自名稱解析'}）｜實際機率 {(actualProb * 100).toFixed(2)}%｜期望 {expectedPoints.toFixed(2)} 點
                    </span>
                  )
                })()}
              </div>

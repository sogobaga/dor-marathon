...
// 303 redirect 過來，網址帶 #credential=<Google ID token>&t=<ticket>（fragment，伺服器端讀不到、
// 不進任何 log）。純客端流程：解析 hash → 立刻清掉 hash → 用 t 兌換 /auth/google/ticket（M2 修法，
// 見該路由與 callback/route.ts 的說明）→ 通過才呼叫後端換發本站 token → 寫入 session → 導回登入前的頁面。
//
...
//
// 【登入 CSRF / M2 修法】光是「這個網址帶著看起來有效的 credential」不足以證明使用者本人剛完成了一次
// 真實的登入流程——有人可能把整段導轉網址轉傳/誘騙受害者開啟，讓受害者被登入成別人指定的帳號。
// 交換後端 token 之前，必須先拿 fragment 裡的 t 去打 /auth/google/ticket，證明「這次載入真的是
// /auth/google/callback 303 redirect 過來的直接延續」；驗證失敗一律視同流程已過期，不嘗試交換 credential。
...
export default function GoogleCompleteClient() {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      // 從 URL fragment 取出 credential 與一次性入場券 t，並立刻清掉 hash（在任何 await 之前）
      // ——不留一次性憑證在網址上。
      const hash = window.location.hash || ''
      const m = /(?:^|[&#])credential=([^&]+)/.exec(hash)
      const mT = /(?:^|[&#])t=([^&]+)/.exec(hash)
      const credential = m ? decodeURIComponent(m[1]) : ''
      const ticket = mT ? decodeURIComponent(mT[1]) : ''
      ...
      try {
        // M2 修法：交換 credential 之前，先證明這次載入真的是 /auth/google/callback 的直接延續
        // （見檔案頂端說明與 auth/google/ticket/route.ts）——不是有人把整段網址轉傳/誘騙受害者開啟
        // 的登入 CSRF。
        let ticketOk = false
        try {
          const ticketRes = await fetch(`/auth/google/ticket?t=${encodeURIComponent(ticket)}`, { cache: 'no-store' })
          const body = await ticketRes.json().catch(() => null)
          ticketOk = ticketRes.ok && body?.ok === true
        } catch {
          ticketOk = false
        }
        if (!ticketOk) {
          setErr('登入流程已過期，請重新登入')
          return
        }
